...
// ===============================
// Sidebar Menu (Workspace Simple)
// - If #mzjSidebar has attribute data-mzj-sidebar="workspace"
//   the sidebar navigation will be replaced with the simple Workspace menu:
//   لوحة التحكم / المشاريع / الأعضاء / الحملات / متابعة السيارات / وايت بورد
(function(){
  function normPath(p){
    try{
      return (p||'').split('?')[0].split('#')[0].toLowerCase();
    }catch(_){ return (p||'').toLowerCase(); }
  }
  function isActive(href){
    const cur = normPath(location.pathname.split('/').pop() || '');
    const target = normPath((href||'').split('/').pop() || '');
    return cur === target || (cur === '' && target === 'dashboard.html');
  }
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    ...
    const mode = sidebar.getAttribute('data-mzj-sidebar');
    ...
    const items = [
      { href:'dashboard.html', label:'لوحة التحكم', icon:'fa-solid fa-gauge-high' },
      { href:'projects.html',  label:'المشاريع',    icon:'fa-solid fa-diagram-project' },
      { href:'members.html',   label:'الأعضاء',     icon:'fa-solid fa-users' },
      { href:'admin.html',     label:'الحملات',     icon:'fa-solid fa-bullhorn' },
      { href:'cars.html',      label:'متابعة السيارات', icon:'fa-solid fa-car' },
      { href:'Whiteboard.html',label:'وايت بورد',   icon:'fa-solid fa-chalkboard' },
    ];

    nav.innerHTML = items.map(it=>{
      const active = isActive(it.href) ? ' active' : '';
      return `
        <a class="side-link${active}" href="${it.href}">
          <i class="${it.icon}"></i>
          <span>${it.label}</span>
        </a>
      `;
